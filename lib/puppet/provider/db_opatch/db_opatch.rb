Puppet::Type.type(:db_opatch).provide(:db_opatch) do
  ...
  def opatch(action)
    ...
    if opatch_auto == true
      ...
      output = `export ORACLE_HOME=#{oracle_product_home_dir}; chmod -R +r #{extracted_patch_dir}; cd #{oracle_product_home_dir}; export _JAVA_OPTIONS="-Djava.io.tmpdir=#{oracle_product_home_dir}"; #{command}`
    else
      ...
      output = `su - #{user} -c 'export ORACLE_HOME=#{oracle_product_home_dir}; cd #{oracle_product_home_dir}; export _JAVA_OPTIONS="-Djava.io.tmpdir=#{oracle_product_home_dir}"; #{command}'`
    end
